...
class _HomePageState extends State<HomePage> with RouteAware {
  ...
  void _startQuizByCategory(BuildContext context, String partKey) {
    ...
    _startQuiz(context, quizzes, partKey);
  }
  ...
  void _startWeaknessReviewByCategory(BuildContext context, String categoryKey) async {
    final navigator = Navigator.of(context);
    final allWeak = await PrefsHelper.getWeakQuestions();
    if (!mounted) return;
    final quizzes = _getQuizzesByKey(categoryKey);
    final qTexts = quizzes.map((q) => q.question).toSet();
    final filtered = allWeak.where((t) => qTexts.contains(t)).toList();
    if (filtered.isEmpty) return;

    final weakQuizzes = QuizData.getQuizzesFromTexts(filtered);
    AdManager.instance.preloadAd('result');
    AdManager.instance.preloadAd('quiz');
    AdManager.instance.preloadInterstitial();
  ...
  String _getCategoryName(String key) {
    switch (key) {
      case 'part1': return '航空法・ルール';
      case 'part2': return '機体・システム';
      case 'part3': return '運航・安全管理';
      case 'part4': return '力学・気象・計算';
      default: return key;
    }
  }
                    ...
                    Expanded(
                      child: OutlinedButton.icon(
                        onPressed: weaknessCount > 0
                            ...
                            : null,
                        icon: const Icon(Icons.history_edu_rounded, size: 16, color: Color(0xFFCC6A43)),
...
class _QuizPageState extends State<QuizPage> {
  ...
  Future<void> _finishQuiz() async {
    ...
    final count = await PrefsHelper.incrementQuizCompletionCount();
    if (count == 3 && mounted) {
      // in_app_review trigger handled by incrementQuizCompletionCount
    }
...
class _ResultPageState extends State<ResultPage> {
  ...
  @override
  Widget build(BuildContext context) {
    final scoreMessage = widget.score == widget.total
        ? 'PERFECT! 🎉'
